...
import (
	...
	repo "github.com/yogavredizon/todolist/repository"
)
...
func main() {
	...
	for run > 0 {
		...
		case 2:
			...
			scanner.Text()
			task := scanner.Text()
			res, err := repo.UpdateTodo(id, task)
		...
		case 3:
			...
			scanner.Text()
			task := scanner.Text()
			res, err := repo.UpdateTime(id, task)
